...
export default function Home() {
  const [currentTab, setCurrentTab] = useState<TabType>('present');
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-cyan-900 dark:via-blue-900 dark:to-teal-800 transition-colors">
        ...
        <section
          ref={addToRefs}
          className="py-16 px-4 bg-gray-100 dark:bg-white/5 backdrop-blur transition-colors"
        >
                ...
                {currentTab === 'present' ? (
                  <>
                    ...
                    <div className="space-y-4">
                      <h4 className="text-xl font-semibold text-gray-800 dark:text-white transition-colors">
                        特徵：
                      </h4>
              ...
              {[
                {
                  icon: FiTarget,
                  title: '目標設定',
                  desc: '明確定義現在的自己與未來的自己，設定具體可達成的階段性目標',
                  gradient: 'from-cyan-400 to-blue-500',
                },
                {
                  icon: FiTrendingUp,
                  title: '進度追蹤',
                  desc: '量化你與未來自己的差距，透過完成子目標逐步縮小差距',
                  gradient: 'from-blue-400 to-teal-500',
                },
                {
                  icon: FiUsers,
                  title: '自我連結',
                  desc: '加強現在與未來自己的連結感，減少拖延並提高行動力',
                  gradient: 'from-teal-400 to-cyan-500',
                },
              ].map(({ icon: Icon, title, desc, gradient }) => (
                <div
                  key={title}
                  className="hover-card bg-white/80 dark:bg-white/10 backdrop-blur border border-gray-200 dark:border-white/20 rounded-2xl p-8 text-center transition-colors"
                >
                  <div
                    className={`w-16 h-16 bg-gradient-to-r ${gradient} rounded-full mx-auto mb-6 flex items-center justify-center`}
                  >
                    <Icon className="h-8 w-8 text-white" />
                  </div>
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4 transition-colors">
                    {title}
                  </h3>
                  <p className="text-gray-600 dark:text-gray-200 transition-colors">
                    {desc}
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col md:flex-row justify-center gap-6">
              ...
              <Link
                scroll={true}
                href="/admin"
                className="inline-flex items-center px-8 py-4 bg-white/10 backdrop-blur dark:text-gray-200 font-semibold rounded-full hover:bg-white/20 transition-all border border-slate-950 dark:border-sky-300 text-black"
              >
                管理目標
              </Link>
            </div>
