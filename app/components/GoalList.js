  ...
  if (!goals || goals.length === 0) {
    return (
      <div className="p-8 text-center">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">還沒有目標</h3>
        <p className="text-gray-600">點擊上方的「新增目標」按鈕來創建你的第一個目標</p>
      </div>
    );
  }
      ...
      {goals.map((goal) => {
        ...
        return (
          <div key={goal.id} className="p-6">
            ...
            {(goal.currentSelfDescription || goal.futureSelfDescription) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 p-4 bg-gray-50 rounded-md">
                {goal.currentSelfDescription && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-700 mb-1">目前的自己</h4>
                    <p className="text-sm text-gray-600">{goal.currentSelfDescription}</p>
                  </div>
                )}
                {goal.futureSelfDescription && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-700 mb-1">未來的自己</h4>
                    <p className="text-sm text-gray-600">{goal.futureSelfDescription}</p>
                  </div>
                )}
              </div>
            )}
            ...
            {/* 子目標列表 */}
            <div>
              <h4 className="text-sm font-medium text-gray-700 mb-3">子目標</h4>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <h5 className={`text-sm font-medium ${
                          subGoal.isCompleted ? 'text-green-800 line-through' : 'text-gray-900'
                        }`}>
                      ...
                      {subGoal.description && (
                        <p className={`text-sm mt-1 ${
                          subGoal.isCompleted ? 'text-green-700' : 'text-gray-600'
                        }`}>
                          {subGoal.description}
                        </p>
                      )}
